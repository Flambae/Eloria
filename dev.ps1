...
Write-Host "Starting Shittim Server (DEV MODE - HOT RELOAD)..." -ForegroundColor Cyan
Start-Sleep -Seconds 1

Write-Host "Starting game server with dotnet watch..." -ForegroundColor Green
# Using dotnet watch run for hot reload
Start-Process cmd -ArgumentList "/k", "cd $baseDir\Shittim-Server && dotnet watch run --project Shittim-Server.csproj" -WindowStyle Normal
...
Write-Host "- Game server (Hot Reload): http://localhost:5000" -ForegroundColor Yellow
Write-Host "- Mitmproxy web interface: http://127.0.0.1:8081" -ForegroundColor Yellow
Write-Host ""
